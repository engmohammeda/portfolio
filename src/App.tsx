...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-800 font-['Alexandria',_sans-serif] selection:bg-blue-600 selection:text-white print:bg-white">
      
      {/* Top Navigation */}
      <nav className="fixed w-full top-0 bg-white/80 backdrop-blur-xl border-b border-slate-200 z-50 transition-all print:hidden">
        <div className="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-6">
            ...
            >
              ...
              <span className="hidden sm:inline">{isRtl ? 'حفظ PDF' : 'Save PDF'}</span>
            </button>
            <button
              onClick={toggleLang}
              className="px-4 py-2 rounded-full border border-slate-200 font-bold text-xs uppercase tracking-widest hover:border-blue-600 hover:text-blue-600 transition-all"
            >
              {lang === 'en' ? 'عربي' : 'EN'}
            </button>
          </div>
      ...
      <section className="pt-32 pb-20 lg:pt-48 lg:pb-32 px-6 relative overflow-hidden print:pt-12 print:pb-8">
        ...
        <div className="max-w-4xl mx-auto text-center">
          <motion.div 
            ...
            className="inline-block mb-8 px-5 py-2.5 rounded-full bg-white border border-slate-200 shadow-sm"
          >
            <span className="bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent font-bold tracking-wide uppercase text-sm">
              {d.header.title}
            </span>
          </motion.div>
          <motion.h1 
            ...
            className="text-4xl sm:text-5xl lg:text-7xl font-extrabold text-slate-900 tracking-tight leading-[1.1] mb-8"
          >
            {d.header.name}
          </motion.h1>
          <motion.p 
            ...
            className="text-lg sm:text-xl lg:text-2xl text-slate-600 font-medium leading-relaxed max-w-3xl mx-auto"
          >
            {d.header.valueProp}
          </motion.p>
          ...
          <motion.div 
            ...
            className="mt-12 flex flex-wrap justify-center gap-6 text-sm font-semibold text-slate-500"
          >
      ...
      <section className="py-20 lg:py-32 bg-slate-900 text-white relative">
        <div className="absolute inset-0 opacity-[0.03]" style={{backgroundImage: 'linear-gradient(white 1px, transparent 1px), linear-gradient(90deg, white 1px, transparent 1px)', backgroundSize: '32px 32px'}}></div>
      ...
      <section className="py-20 lg:py-32 bg-white">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <motion.div {...fadeUp} className="flex flex-col sm:flex-row justify-center sm:justify-between items-center mb-12 gap-6 bg-slate-50 p-4 rounded-2xl border border-slate-100">
            <div className="flex flex-wrap gap-2 justify-center">
              {allCategories.map(cat => (
                <button
                  key={cat}
                  onClick={() => setFilterCategory(cat)}
                  className={`px-4 py-2 rounded-full text-sm font-semibold transition-all ${filterCategory === cat ? 'bg-blue-600 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-200 border border-slate-200'}`}
                >
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-white border border-slate-200 text-sm font-semibold text-slate-600 hover:bg-slate-50 transition-colors shrink-0"
            >
              ...
              {filteredProjects.map((project, idx) => (
                ...
                >
                  ...
                  <div className="w-full lg:w-1/2 space-y-8">
                    <div>
                      <h3 className="text-2xl lg:text-3xl font-bold text-slate-900 mb-2">{project.name}</h3>
                      <div className="flex items-center gap-3">
                        <p className="text-blue-600 font-semibold tracking-wide">{project.role}</p>
                        <span className="text-sm font-mono text-slate-400 bg-slate-100 px-2 py-0.5 rounded">{project.date}</span>
                      </div>
                    </div>
                  
                  <div className="space-y-6">
                    <div className="bg-red-50/50 border border-red-100 rounded-2xl p-6">
                      <h4 className="text-xs font-bold uppercase tracking-widest text-red-500 mb-3">{isRtl ? 'التحدي / المشكلة' : 'The Problem'}</h4>
                      <p className="text-slate-700 leading-relaxed font-medium">{project.context}</p>
                    </div>
                    
                    <div className="bg-blue-50/50 border border-blue-100 rounded-2xl p-6 relative">
                      <h4 className="text-xs font-bold uppercase tracking-widest text-blue-600 mb-3">{isRtl ? 'الحل الهندسي' : 'Engineered Solution'}</h4>
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h4 className="text-xs font-bold uppercase tracking-widest text-emerald-600 mb-2">{isRtl ? 'الأثر / النتائج' : 'The Impact'}</h4>
                        <p className="text-slate-800 font-bold leading-relaxed">{project.impact}</p>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-24">
            ...
            <motion.div {...fadeUp} className="lg:col-span-7">
              <div className="flex items-center gap-4 mb-12">
                ...
                <h2 className="text-3xl font-bold text-slate-900">{d.experience.title}</h2>
              </div>
              
              <div className="space-y-12 relative before:absolute before:inset-0 before:ms-[23px] before:h-full before:w-px before:bg-slate-200">
                {d.experience.jobs.map((job, idx) => (
                  <div key={idx} className="relative ps-16">
                    <span className="absolute start-[19px] top-2 w-2.5 h-2.5 rounded-full bg-blue-600 shadow-[0_0_0_6px_white] ring-1 ring-slate-200"></span>
                    
                    <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 gap-4">
                        <h3 className="text-xl font-bold text-slate-900">{job.title}</h3>
                        <span className="inline-flex items-center justify-center px-4 py-1.5 rounded-full bg-slate-100 text-slate-600 text-xs font-bold tracking-wider font-mono">
                          {job.date}
                        </span>
                      </div>
                      <h4 className="text-sm font-bold text-blue-600 mb-6">{job.company}</h4>
                      
                      <ul className="space-y-4">
                        {job.points.map((pt, i) => (
                          <li key={i} className="flex gap-4 text-[15px] leading-relaxed text-slate-600">
                            <span className="mt-[8px] w-1.5 h-1.5 rounded-full bg-slate-300 shrink-0"></span>
                            <span>{pt}</span>
                          </li>
                        ))}
                      </ul>
            ...
            <motion.div {...fadeUp} className="lg:col-span-5 space-y-12">
              <div>
                <div className="flex items-center gap-4 mb-10">
                  ...
                  <h2 className="text-3xl font-bold text-slate-900">{d.competencies.title}</h2>
                </div>

                {d.competencies.chartData && (
                  <div className="h-72 mb-8 bg-white appearance-none rounded-3xl border border-slate-200 shadow-sm p-4">
                    <ResponsiveContainer width="100%" height="100%">
                      <RadarChart cx="50%" cy="50%" outerRadius="70%" data={d.competencies.chartData}>
                        ...
                        <Tooltip wrapperClassName="text-sm font-semibold text-slate-700" contentStyle={{ borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1)' }} />
                      </RadarChart>
                    </ResponsiveContainer>
                  </div>
                )}
                ...
                <div className="space-y-6">
                  {d.competencies.categories.map((cat, i) => (
                    <div key={i} className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                      <h4 className="text-sm font-bold text-slate-900 mb-4">{cat.name}</h4>
              ...
              <div>
                <div className="flex items-center gap-4 mb-10">
                  ...
                  <h2 className="text-3xl font-bold text-slate-900">{d.workSamples.title}</h2>
                </div>
                ...
                <div className="grid gap-4">
                  {d.workSamples.samples.map((sample, i) => (
                    <div 
                      key={i} 
                      className="group flex items-center justify-between p-5 rounded-2xl bg-white border border-slate-200 shadow-sm hover:border-blue-300 hover:shadow-md transition-all cursor-pointer"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h5 className="font-bold text-slate-900 text-sm mb-1">{sample.title}</h5>
                          <p className="text-xs text-slate-500 font-medium">{sample.desc}</p>
                        </div>
                      </div>
                      <div className="w-10 h-10 rounded-full border border-slate-100 flex items-center justify-center text-slate-400 group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 transition-all">
      ...
      {/* CTA Footer */}
      <section className="py-20 lg:py-32 bg-blue-600 text-white text-center print:hidden">
        <div className="max-w-3xl mx-auto px-6">
          <motion.div {...fadeUp}>
            ...
            <a 
              ...
              className="inline-flex items-center gap-3 bg-white text-blue-600 px-8 py-4 rounded-full font-bold text-lg hover:bg-slate-50 hover:scale-105 active:scale-95 transition-all shadow-xl shadow-blue-900/20"
            >
      ...
      <footer className="py-8 text-center bg-slate-900 text-slate-500 text-sm font-medium print:bg-white print:text-slate-400">
        {d.education.details} • {new Date().getFullYear()}
      </footer>
      ...
      <AnimatePresence>
        {showPrintWarning && (
          <motion.div 
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 50 }}
            className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-slate-900 text-white px-6 py-3 rounded-full shadow-2xl z-[100] flex items-center gap-3 text-sm font-bold border border-slate-700"
          >
